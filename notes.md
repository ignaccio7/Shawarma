...
Para que no falle con los tipos en node
...
Para recibir los archivos y convertirlos a json
```bash
    npm install multer convert-csv-to-json
...
Para cambiar el puerto por defecto en react
```bash
"scripts": {
    "dev": "vite",
---
...
# Para que las dependencias nos sirvan para todo el proyecto.

1. Creamos un package.json en la raiz de nuestro proyecto.
2. Copiamos la configuracion del eslint que teniamos en el frontend al package de la carpeta raiz.
3. Movemos tambien el archivo **.eslintrc.cjs** del frontend a la carpeta raiz
4. Le indicamos en el archivo anterior que tambien queremos que revise node.

# Para montar un monorepositorio

Si queremos que se levante tanto el backend como el frontend colocando

```bash
    npm run dev
```

> Necesitamos un Workspace

Añadimos en el archivo package.json de nuestra raiz el siguiente contenido:
